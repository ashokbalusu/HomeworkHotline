...
namespace HomeworkHotline.Controllers
{
    ...
    public class AccountController : Controller // : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    if (returnUrl == null) returnUrl = "~/Timesheet";
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, FirstName = model.FirstName, LastName = model.LastName, HireDate = model.HireDate, PhoneNumber = model.PhoneNumber, MNPSEmployeeNo = model.MNPSEmployeeNo, EndDate = model.EndDate, Volunteer = model.Volunteer, MainSubject = model.MainSubject, SubjectOther = model.SubjectOther, GradeLevel = model.GradeLevel   };
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    //we are not currently sending an email confirmation on registration so we do not need to check this value.
                ...
                catch (Exception e)
                {
                    ...
                    ViewBag.Message = "There was an error sending the reset link. Please check the email and try again. If you continue to have problems, please contact the administrator.";
                    return View(model);
                }
            }
